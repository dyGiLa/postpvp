...
@smproxy.filter(label="Hoshen-Kopelman Filter")
@smproperty.input(name="Input")
class HoshenKopelman_Class(VTKPythonAlgorithmBase):
    def __init__(self):
    ...
    def RequestData(self, request, inInfo, outInfo):
        ...
        # vtk_output = vtkDataSet.GetData(outInfo)
        # vtk_output.ShallowCopy(data.VTKObject)
        # vtk_output.SetExtent(0, dim-1, 0, dim-1, 0, dim-1)
        ...
        # print("Input points:", data.VTKObject.GetNumberOfPoints())  # Should be > 0
        ...
        # output.PointData.AddArray(original_indices)
        # # output.GetRowData().AddArray(original_indices)
        # output.PointData.AddArray(vtk_labels)
        # output.PointData.AddArray(vtk_labels_relabeled)
        # output.PointData.AddArray(vtk_dim)
