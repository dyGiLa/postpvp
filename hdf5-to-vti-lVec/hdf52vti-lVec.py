...
## fields path
l1_path = "/mesh/fields/l1/values"
l2_path = "/mesh/fields/l2/values"
l3_path = "/mesh/fields/l3/values"
...
for idx, rankhdf5 in enumerate(rank_h5_files):
    ...
    l1_values = h5f[l1_path][()].astype('float')
    l2_values = h5f[l2_path][()].astype('float')
    l3_values = h5f[l3_path][()].astype('float')        

    # covert to vtk_array
    vtk_l1_array = numpy_support.numpy_to_vtk(l1_values)
    vtk_l1_array.SetName("l1")

    vtk_l2_array = numpy_support.numpy_to_vtk(l2_values)
    vtk_l2_array.SetName("l2")

    vtk_l3_array = numpy_support.numpy_to_vtk(l3_values)
    vtk_l3_array.SetName("l3")
    
    
    # Attach to image data
    pd = vtk_grid.GetPointData()

    pd.AddArray(vtk_l1_array)
    pd.AddArray(vtk_l2_array)
    pd.AddArray(vtk_l3_array)    
    

    # Optionally make one of them the default for visualization
    pd.SetScalars(vtk_l1_array)
